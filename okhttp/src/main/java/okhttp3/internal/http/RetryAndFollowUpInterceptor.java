...
public final class RetryAndFollowUpInterceptor implements Interceptor {
     ...
     * sent.
     */
    //判断发生的异常情况是否满足恢复重试的条件。只有当请求体被缓冲或者请求体在请求发送之前发生故障时，才可以恢复带有请求体的请求。
    ...
    private Request followUpRequest(Response userResponse, @Nullable Route route) throws IOException {
        ...
        switch (responseCode) {
            ...
            case HTTP_CLIENT_TIMEOUT://408 实际很少用到，一般需要重复发送一个相同的请求。一般在HA中可能会用到。这种情况，直接使用原来的url以及body即可
                ...
                //根据返回的信息设置了Retry-After(表示多久之后才能进行重试)，那么okhttp会直接停止，可能需要用户去进行处理了。
            ...
            case HTTP_UNAVAILABLE://503  服务器节点有问题
                if (userResponse.priorResponse() != null && userResponse.priorResponse().code() == HTTP_UNAVAILABLE) {
                    // We attempted to retry and got another timeout. Give up.
